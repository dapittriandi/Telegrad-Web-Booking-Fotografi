...
.breadcrumb-nav i { font-size: .6rem; color: var(--dim); }
...
.btn-back {
    ...
    padding: 9px 18px; border-radius: 8px; font-size: .78rem; font-weight: 600;
    color: var(--muted); border: 1px solid var(--border);
    text-decoration: none;
    transition: border-color .2s, color .2s, background .2s;
}
.btn-back:hover { border-color: var(--gold-border); color: var(--gold); background: var(--gold-dim); }

/* ─── Layout ─────────────────────────────────────────────── */
.detail-section { padding: 36px 0 80px; }
...
.info-card:hover { border-color: var(--gold-border); box-shadow: var(--shadow-sm); }
...
[data-theme="light"] .info-card-head { background: rgba(0,0,0,.025); }
...
.order-created { font-size: .76rem; color: var(--muted); margin-top: 3px; display: flex; align-items: center; gap: 5px; }
...
.prog-step.done   .prog-icon { border-color: var(--success); background: rgba(76,175,130,.12); color: var(--success); }
.prog-step.active .prog-icon { border-color: var(--gold); background: var(--gold-dim); color: var(--gold); }

.prog-label { font-size: .66rem; font-weight: 600; letter-spacing: .06em; text-transform: uppercase; color: var(--muted); }
...
.pay-verified{ background: rgba(76,175,130,.12); border: 1px solid rgba(76,175,130,.25); color: var(--success); }
.pay-rejected{ background: rgba(224,92,92,.12);  border: 1px solid rgba(224,92,92,.25);  color: var(--danger); }
.pay-pending { background: rgba(224,169,53,.12); border: 1px solid rgba(224,169,53,.25); color: #e0a935; }
...
.alert-warning { background: rgba(224,92,92,.06);  border: 1px solid rgba(224,92,92,.2);   color: #f08080; }
...
.star-input label { font-size: 1.8rem; color: var(--border-strong); cursor: pointer; transition: color .2s; }
...
.review-text {
    ...
    line-height: 1.7; font-style: italic; margin-top: 12px;
    transition: background .3s, border-color .3s;
}
...
.rating-form-textarea {
    ...
    transition: border-color .2s, background .3s; margin-top: 16px;
    font-family: inherit;
}
...
.btn-rating-submit:hover { opacity: .88; transform: translateY(-2px); }

.skip-rating-link { font-size: .78rem; color: var(--muted); text-decoration: none; transition: color .2s; }
...
.btn-sidebar-gold {
    ...
    transition: opacity .2s, transform .2s; margin-top: 12px;
}
.btn-sidebar-gold:hover { opacity: .88; color: #0d0d0d; transform: translateY(-2px); }
...
.btn-sidebar-outline:hover { background: var(--gold-dim); color: var(--gold-light); }
...
.ss-all-link:hover { color: var(--gold); }
...
.payment-empty i { font-size: 2rem; color: var(--gold-border); display: block; margin-bottom: 10px; }
</style>
@endpush
    ...
    <div class="container">
        <div class="page-header-inner">
            <div class="breadcrumb-nav">
                ...
                <a href="{{ route('customer.orders') }}">Riwayat Order</a>
                <i class="bi bi-chevron-right"></i>
        ...
        <div class="col-lg-8">
            ...
            <div class="info-card" data-aos="fade-up">
                ...
                <div class="info-card-body">
                    @if($order->payment)
                        ...
                        @endif
                        @if($ps === 'rejected')
                        <div class="alert-box alert-warning" style="margin-top:14px;margin-bottom:0;">
                            ...
                            <div>
                                ...
                                <a href="{{ route('customer.payment.create', $order->id) }}"
                                   style="color:var(--gold);display:inline-flex;align-items:center;gap:5px;margin-top:8px;font-weight:700;text-decoration:none;">
                    ...
                    @else
                        <div class="payment-empty">
                            ...
                            <div style="margin-top:14px;">
                                <a href="{{ route('customer.payment.create', $order->id) }}" class="btn-sidebar-gold" style="display:inline-flex;width:auto;padding:10px 22px;">
            ...
            <div class="info-card" data-aos="fade-up">
                ...
                <div class="info-card-body">
                    ...
                    @else
                        <p style="font-size:.85rem;color:var(--muted);margin-bottom:16px;line-height:1.7;">
                        ...
                        <form action="{{ route('customer.rating.store') }}" method="POST">
                            ...
                            @error('rating')
                            <div style="font-size:.74rem;color:var(--danger);margin-top:6px;">{{ $message }}</div>
                            @enderror
                            <textarea name="review" class="rating-form-textarea"
                                      placeholder="Tulis ulasan kamu... (opsional)">{{ old('review') }}</textarea>
                            @error('review')
                            <div style="font-size:.74rem;color:var(--danger);margin-top:4px;">{{ $message }}</div>
                            @enderror
        ...
        <div class="col-lg-4" data-aos="fade-left">
            <div class="sidebar-summary">
                ...
                <div class="ss-body">
                    ...
                    <a href="{{ route('customer.payment.create', $order->id) }}" class="btn-sidebar-gold">
                        <i class="bi bi-credit-card"></i> Bayar Sekarang
                    </a>
                    @endif

                    <a href="{{ route('packages.detail', $order->package_id) }}" class="btn-sidebar-outline">
                        <i class="bi bi-arrow-repeat"></i> Pesan Lagi
                    </a>

                    <a href="{{ route('customer.orders') }}" class="ss-all-link">
